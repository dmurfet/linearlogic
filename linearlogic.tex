...
The outline of the article is as follows: in Section \ref{section:sketch} we give a lightning tour of linear logic by presenting it as an interesting extension of the language of symmetric closed monoidal categories, but omitting all details. Part of the reason why this extension is so interesting is its close connection to concepts of computer science, so in Section \ref{section:lambda_calc} we give a short introduction to the $\lambda$-calculus, an archetypal programming language, in order to be able to make those connections explicit and introduce the basic idea of rewrite rules in a simple context. In Section \ref{section:intro_ll} we define linear logic and study some examples of proofs, before turning in Section \ref{section:diagrammatics} to the semantics in vector spaces. In Section \ref{section:cut_elim} we study cut-elimination, and then in Section \ref{section:second} we discuss second-order linear logic. In Appendix \ref{section:appendix_cut_elim} we give a detailed example of cut-elimination and in Appendix \ref{section:example_lifting} we examine tangent maps in connection with proofs in linear logic. 
...
Linear logic was introduced by Girard in the 1980s \cite{girard_llogic} and it has been actively studied ever since, in both computer science and mathematical logic. Its unique features as a logic make it naturally connected with algebra (broadly interpreted). At its root this connection between the two subjects is linguistic: symmetric closed monoidal categories are ubiquitous in algebra, and their formal language is a subset of linear logic. 
...
Happily, symmetric closed monoidal categories are the \emph{least} interesting part of linear logic. Things become more interesting when we start to talk about natural operations on morphisms in $\cat{C}$ that cannot be encoded by a diagram like \eqref{eq:construct_comp_catt}. For example, there is the operation which takes as input an object $a$, an endomorphism $f: a \lto a$, and returns the square $f \circ f$, and this has no such description (why?). However, this operation \emph{is} described by a proof in linear logic, namely:
\begin{center}
\AxiomC{}
\UnaryInfC{$A \vdash A$}
\AxiomC{}
...
\end{center}
This proof involves a new connective $!$ called the \emph{exponential}, but we recognise that until the fourth line, this is the earlier proof with $A$ substituted for $B,C$. Since that proof was presenting the operation of composition, it only takes a small leap of imagination to see the proof \eqref{church_2_intro} as a gadget which takes a morphism $f: a \lto a$ and duplicates it, feeding $f,f$ into the composition operation to obtain $f \circ f$.
...
WHY are we talking about the calculus?
...
We have now defined the $\lambda$-calculus and introduced our basic example of a program, the Church numeral $T$. The emphasis in this article is on the structure of sets of proofs, and this is turns out to be closely related to the structure of sets of \emph{programs}, that is, $\lambda$-terms. Let $D$ denote the set of all $\beta$-equivalence classes of $\lambda$-terms. This is a fabulously complicated set: consider that for example $T$ defines a function
\begin{gather*}
T: D \lto D,\\
[M] \mapsto [(T \, M)]\,.
\end{gather*}
and the same is true of any term. The discovery of the first mathematical model of this structure by Dana Scott \cite{scott} was an important milestone in the development of computer science, and it marked the beginning of the field called \emph{denotational semantics}. 

For our purposes, however, the full $\lambda$-calculus is too complicated and its denotational semantics too far from algebra. The \emph{simply-typed} $\lambda$-calculus is a variant which is more closely related to linear logic. The reader familiar with programming will know that functions in many programming languages are \emph{typed} by their input and output specification. For instance, a function computing the $n$th Fibonacci number in the language C takes an input of type $\inta$ and returns an output of type $\inta$, and thus we say the program itself has type $\inta \rightarrow \inta$. In the simply-typed $\lambda$-calculus terms are all annotated with their type. For example the annotated version of the Church numeral $T$ is
\[
...
In what sense is this proof an avatar of the numeral $2$? In the context of the $\lambda$-calculus we appreciated the relationship between the term $T$ and the numeral $2$ only after we saw how $T$ interacted with other terms $M$ by forming the function application $(T \, M)$ and then finding a normal form with respect to $\beta$-equivalence in \eqref{eq:beta_reduc_dup}. 

The analogue of function application in linear logic is the cut rule. The analogue of $\beta$-equivalence is an equivalence relation on the set of proofs of any sequent, which we write as $\rho =_{cut} \delta$ and communicate by saying that $\rho$ and $\delta$ are \emph{equivalent under cut-elimination}. We will have more to say about cut-elimination in Section \ref{section:cut_elim}, but for now let us examine how it gives a dynamic meaning to the proof $\church{2}_A$. 
...
\end{tabular}
\end{center} 
which we is essentially the square $\pi \l \pi$.
\end{example}

With $\pi$ playing the role of the term $M$ in Section \ref{section:lambda_calc}, we see the close analogy between the program $T$ of $\lambda$-calculus and the proof $\church{2}_A$ of linear logic, with the cut \eqref{cut_against_2_early} playing the role of $(T \, M)$ and cut-elimination the role of $\beta$-reduction. This is one aspect of the \emph{Curry-Howard correspondence} \cite[\S 6.5]{selinger} which relates programs in the simply-typed $\lambda$-calculus (and its extensions) to proofs in intuitionistic logic (and its extensions) with cut-elimination playing the role of $\beta$-reduction. We will not go into details here, but there is a chain of embeddings
\[
...
As we have already mentioned in the previous section, on the set of proofs of any sequent $\Gamma \vdash A$ there is an equivalence relation called \emph{cut-elimination}. As was the case with the set of $\lambda$-terms modulo $\beta$-equivalence, the structure of the set of proofs in linear logic modulo cut-elimination is quite complicated, and one way to understand this structure is to try to model it in simpler mathematical structures, like vector spaces and linear maps. These models are called \emph{semantics} of linear logic.
...
In this paper denotations all take place in the category $\cat{V}$ of $k$-vector spaces where $k$ is an algebraically closed field of characteristic zero. 

We explain the denotations of formulas now, and leave the denotation of proofs to the next section. To this end, for a vector space $V$ let ${!} V$ denote the cofree cocommutative coalgebra generated by $V$. There is a universal morphism $d: {!} V \lto V$. We will discuss the explicit form of this coalgebra in the next section; for the moment it is enough to know that it exists and is determined up to unique isomorphism.
...
In the previous section we associated to each formula $A$ a vector space $\den{A}$. In this section we complete the construction of the vector space semantics of linear logic by assigning to each proof $\pi$ of a sequent $\Gamma \vdash B$ a linear map $\den{\Gamma} \lto \den{B}$. The essential features of this assignment can be seen already in the following simple example:
...
The first theorem is that the relation $\rightsquigarrow$ is \emph{weakly normalising}, which means:
...
\begin{remark} It is possible that multiple rewrite rules apply to a given proof, so that there is more than one sequence \eqref{eq:proof_pi} with $\pi$ as its starting point. This raises the natural questions: does \emph{every} sequence of rewrites starting from $\pi$ terminate with a cut-free proof, and is the end result $\widetilde{\pi}$ independent of all the choices? This property of a rewrite rule is called \emph{strong normalisation}. Unfortunately in its sequent calculus presentation linear logic does \emph{not} satisfy strong normalisation and in particular it is not true that each cut-elimination equivalence class contains a unique cut-free proof.

However it is true that two cut-free proofs in the same equivalence class are ``essentially the same'' in the sense that they are assigned the same proof-net \cite[Remark 4.19]{pagani}. Indeed cut-elimination of proof-nets satisfies strong normalisation. For first-order linear logic as presented here, this is proven in Girard's original paper \cite{girard_llogic} but for second-order linear logic there are gaps in the proof given there; see \cite{pagani}.
\end{remark}
