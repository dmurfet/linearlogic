...
\section{Linear logic as a language}\label{section:sketch}
...
Happily, symmetric closed monoidal categories are the \emph{least} interesting part of linear logic and there are plenty of algorithms that \emph{cannot} be encoded in a diagram like \eqref{eq:construct_comp_catt}. For example, the operation that takes as input an object $a$, an endomorphism $f: a \lto a$, and returns the square $f \circ f$ has no such description (why?). However, this operation \emph{is} described by a proof in linear logic, namely:
\begin{center}
\AxiomC{}
\UnaryInfC{$A \vdash A$}
\AxiomC{}
